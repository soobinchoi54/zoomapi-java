...
<br> It is important to note that Zoom API's message components can be performed in two contexts: messages sent to a channel and messages sent to a member. For the purpose of this project, we provide one table that stores message data sent to both a channel and a member by treating a member as a channel (channelId and channelName columns will store the member's email address).We will be able to query from our cache system for both types of messages in our bot application. 
